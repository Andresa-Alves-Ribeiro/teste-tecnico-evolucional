...
jest.mock('./TeacherHeader', () => () => <div data-testid="teacher-header">Teacher Header</div>);
jest.mock('./TeacherStats', () => ({ totalTeachers, totalRelationships }: any) => (
...
jest.mock('./TeacherFilters', () => ({ onDegreeChange, onClassChange, onAddClick }: any) => (
...
jest.mock('./AddRelationshipDialog', () => ({ open, onClose, onAdd }: any) => (
  open ? (
...
jest.mock('../common/DataTable', () => ({ items, columns }: any) => (
...
describe('TeacherManagement Component', () => {
  ...
  it('applies correct styling to degree and class cells', () => {
    ...
    // Check if cells have the expected styling classes
